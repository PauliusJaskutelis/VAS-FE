...
describe('ImageCard', () => {
    it('renders with filename and fetches preview image', async () => {
        render(_jsx(ImageCard, { imageId: "123", filename: "test.png" }));
        expect(screen.getByText('test.png')).toBeInTheDocument();
        await waitFor(() => expect(global.fetch).toHaveBeenCalledWith('http://localhost:8080/api/images/123', expect.objectContaining({
    ...
    it('calls onDelete when delete icon is clicked', async () => {
        ...
        render(_jsx(ImageCard, { imageId: "123", filename: "deletable.png", onDelete: handleDelete }));
    ...
    it('handles fetch failure gracefully', async () => {
        ...
        render(_jsx(ImageCard, { imageId: "error-test" }));
        await waitFor(() => {
            expect(errorSpy).toHaveBeenCalledWith(expect.stringContaining('❌ Failed to load image preview:'), 'fetch failed');
        });
