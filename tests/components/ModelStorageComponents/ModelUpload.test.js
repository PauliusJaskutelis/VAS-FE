...
describe('ModelUpload', () => {
    ...
    it('shows error if fields are empty on upload', async () => {
        render(_jsx(ModelUpload, { open: true, onClose: mockOnClose, onModelUpload: mockOnModelUpload }));
    ...
    it('uploads a model successfully and triggers callbacks', async () => {
        ...
        uploadModel.mockResolvedValueOnce(mockResponse);
        render(_jsx(ModelUpload, { open: true, onClose: mockOnClose, onModelUpload: mockOnModelUpload }));
        ...
        await waitFor(() => {
            ...
            expect(uploadModel).toHaveBeenCalledWith(mockFile, 'MyModel', expect.any(String));
        });
    });
});
